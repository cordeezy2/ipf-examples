...
def join_table_page():
    ...
    right_table_options = st.multiselect(
        "Select right table", list_of_tables, max_selections=1
    )
    right_table_options_columns = ipf.get_columns(right_table_options[0])
    right_table_options_columns.remove("id")
    right_table_options_columns = st.multiselect(
        "Select columns for right table", right_table_options_columns
    )
    right_table_filter = st.text_input("Filter for right table", value="{}")

    left_table_options = st.multiselect(
        "Select left table", list_of_tables, max_selections=1
    )
    left_table_options_columns = ipf.get_columns(left_table_options[0])
    left_table_options_columns.remove("id")
    left_table_options_columns = st.multiselect(
        "Select columns for left table", left_table_options_columns
    )
    left_table_filter = st.text_input(
        "Filter for left table", value="{}", help='{"and": [{"mode": ["eq","trunk"]}]}'
    )

    columns = set(column for column in ipf.get_columns(right_table_options[0])) & set(
        column for column in ipf.get_columns(left_table_options[0])
    )
    ...
    table_data_left = ipf.fetch_all(
        left_table_options[0],
        export="df",
        columns=left_table_options_columns,
        filters=json.loads(left_table_filter),
    )
    table_data_right = ipf.fetch_all(
        right_table_options[0],
        export="df",
    ...
    try:
        ...
        st.write(
            f"You more than likely have a column mismatch, please check and try again. {e}"
        )
        joined_data = None
    st.markdown("### Joined Data")
    st.write(joined_data)

    snapshot_selection = [snap for snap in ipf.get_snapshots()]
    for snap_id in ["$last", ipf.snapshot_id]:
        if snap_id in snapshot_selection:
            snapshot_selection.remove(snap_id)

    snapshot_selection = st.multiselect(
        "Select IP Fabric Snapshot", snapshot_selection, max_selections=1
    )
    compare_table_data_left = ipf.fetch_all(
        left_table_options[0],
        export="df",
        snapshot_id=snapshot_selection[0],
        columns=left_table_options_columns,
        filters=json.loads(left_table_filter),
    )
    compare_table_data_right = ipf.fetch_all(
        right_table_options[0],
        export="df",
        snapshot_id=snapshot_selection[0],
        columns=right_table_options_columns,
    ...
    compare_joined_data = pd.merge(
        compare_table_data_left, compare_table_data_right, on=columns, how="outer"
    )
